# PRD Changelog — Claw Bot Army v1.0 → Akasha v1.2
...
This document consolidates all changes from the original v1.0 PRD through to the current v1.2. Two major release cycles are covered.
...
## 4. New Section — LLM Backend Strategy (Section 8, added v1.2)

Did not exist in v1.0 or v1.1.

**MVP position.** Single configured LLM backend. All inference routed to one provider and model. The Akashic Library schema carries a reserved `llm_backend` field written as null for all MVP entries.

**Why multi-LLM is out of MVP scope.** Model selection is a fitness variable, not neutral infrastructure. Two agents with identical SOUL.md configurations but different backends produce different outputs. If the council attributes fitness signals to the soul without recording which model executed it, the attribution is incomplete and potentially misleading. A soul promoted on Opus runs may perform poorly on Haiku. Multi-model comparison also requires cost-adjusted fitness scoring, which does not exist in MVP.

**Post-MVP roadmap.** Three changes activate when multi-LLM is introduced: the `llm_backend` field populates with model identifier strings; Army Builder exposes model tier selection (Standard / Premium / Economy); fitness scoring gains a cost-efficiency dimension normalising performance against model cost.

**SOUL.md vs system prompt boundary.** Formally clarified for the first time. SOUL.md is the evolvable behavioural constitution — any instruction that shapes agent behaviour belongs here. The system prompt is a thin fixed wrapper for tool call syntax and output format requirements only. It does not vary between agents and is not part of the evolvable soul. Behavioural directives that drift into the system prompt become invisible to the soul evolution engine — they cannot be attributed, versioned, or mutated.

**Schema reserve.** `llm_backend` field reserved in the Akashic Library schema with a partial index (`WHERE llm_backend IS NOT NULL`). MVP null entries add zero index overhead. When multi-LLM data populates the field post-launch, the index is already in place.
...
*Claw Bot Army PRD v1.0 → Akasha PRD v1.2*
