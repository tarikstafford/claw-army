...
function buildDevilPrompt(ctx: CouncilContext): string {
  ...
  // DA gets ALL traces — full picture including failures and rejections
 ...
 * Uses Google gemini-2.5-flash (heterogeneous provider per CNCL-03). Temperature 0.5 for
 ...
 *   Override the LLM's value to guarantee structural consistency (CNCL-05).
 */
export async function runDevilsAdvocate(ctx: CouncilContext): Promise<DevilsAdvocateOutput> {
  ...
  // based on challenge severity. This is a structural guarantee (CNCL-05):
  // any strong challenge must trigger human review, regardless of LLM self-assessment.
