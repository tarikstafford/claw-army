 ...
 * This means the agent's self-attribution is considered unreliable for that decision.
 */
...
function buildSoulAnalystPrompt(ctx: CouncilContext): string {
  ...
  // Filter to high-confidence directive attributions only
 ...
 * Uses Anthropic claude-sonnet-4-6. Temperature 0.2 for rigorous, consistent analysis.
...
export async function runSoulAnalyst(ctx: CouncilContext): Promise<SoulAnalystOutput> {
  ...
  // This guarantees structural consistency regardless of LLM judgment on the boolean.
