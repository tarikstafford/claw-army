...
import {
  ...
  getExecution,
  transitionExecution,
  startEvolutionCampaign,
  EVOLUTION_MAX_ITERATIONS_CEILING,
...
export const executionsRoutes: FastifyPluginAsyncTypebox = async (fastify) => {
  ...
  fastify.post(
    "/",
    {
      schema: {
        body: Type.Object({
          ...
          // Evolution Campaign opt-in (Karpathy Loop, issue #74). Defaults to false:
          // a standalone execution is a 1-iteration campaign. Set to true to enable
          // auto-iteration against the same objective, capped at `maxIterations`.
          enableEvolution: Type.Optional(Type.Boolean()),
          maxIterations: Type.Optional(
            Type.Integer({
              minimum: 1,
              maximum: EVOLUTION_MAX_ITERATIONS_CEILING,
            }),
          ),
          // Cumulative cost cap across the whole campaign. Nullable — if omitted,
          // only per-execution caps apply.
          campaignBudgetCapCents: Type.Optional(Type.Integer({ minimum: 0 })),
        }),
        response: {
          201: Type.Object({
            executionId: Type.String({ format: "uuid" }),
            status: Type.Literal("pre_flight"),
            // Present only when enableEvolution=true.
            evolutionCampaignId: Type.Optional(Type.String({ format: "uuid" })),
          }),
          400: Type.Object({
            error: Type.String(),
    ...
    async (request, reply) => {
      ...
      // 3a. Evolution campaign opt-in path (issue #74): create a campaign,
      //     its first execution, and seed iteration 1 in one call.
      // 3b. Standard path: single ad-hoc execution, no campaign linkage.
      let result: {
        executionId: string;
        status: "pre_flight";
      ...
      try {
        if (enableEvolution === true) {
          // Evolution campaigns don't support objectiveId/campaignType fields
          // from the legacy API — the campaign IS the campaign concept now.
          const campaignResult = await startEvolutionCampaign({
            objective,
            maxBots,
            ...
            allowedDomains,
            projectId,
            maxIterations: maxIterations ?? EVOLUTION_MAX_ITERATIONS_CEILING,
            campaignBudgetCapCents,
          });
          result = {
            executionId: campaignResult.executionId,
            status: "pre_flight",
            evolutionCampaignId: campaignResult.campaignId,
          };
        } else {
          const standardResult = await createExecution({
            objective,
            maxBots,
            budgetCapCents: budgetCapCents ?? 0,
            runtimeLimitSeconds: runtimeLimitSeconds ?? 3600,
      ...
      // 4. Return 201 with execution ID (and campaign ID if opted in)
      ...
      setImmediate(async () => {
        try {
          ...
          const resolvedCampaignType =
            campaignType ?? (objectiveId ? "campaign" : "ad_hoc");
          ...
          fastify.log.error(
            { err, executionId },
            "Failed during Ring Leader spawn",
          );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      type AgentClassInfo = {
        agentClass: "Novice" | "Understudy" | "Artisan" | "Retired";
        isPioneer: boolean;
      };
      ...
      const leaderboard = await Promise.all(
        botRows.map(async (bot) => {
          const [completedRow] = await db
            ...
            .where(
              and(
                eq(tasks.executionId, id),
                eq(tasks.claimedByBotId, bot.botId),
          ...
          return {
            botId: bot.botId,
            compositeScore: bot.compositeScore
              ? Number(bot.compositeScore)
              : null,
            tier: bot.tier,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      await Promise.allSettled(
        activeBots.map((b) => stopBot(b.botId, "terminated")),
      );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (execution.status !== "pre_flight") {
        return reply
          .code(409)
          .send({ error: "Execution is not awaiting pre-flight confirmation" });
      }
      ...
      if (
        !runRow?.populationManifest ||
        !Array.isArray(runRow.populationManifest) ||
        runRow.populationManifest.length === 0
      ) {
        return reply
          .code(409)
          .send({ error: "Population manifest not yet assembled" });
      }

      const transitioned = await transitionExecution(
        id,
        "pre_flight",
        "queued",
      );
      if (!transitioned)
        return reply.code(409).send({ error: "Status transition conflict" });
      ...
      setImmediate(async () => {
        try {
          ...
          fastify.log.error(
            { err, executionId: id },
            "Agent spawn failed after confirm",
          );
        }
      });
    },
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (execution.status !== "pre_flight") {
        return reply
          .code(409)
          .send({ error: "Execution is not in pre_flight status" });
      }
